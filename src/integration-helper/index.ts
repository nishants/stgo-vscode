...
const SCREENSHOT_API_URL =
  "http://st-integration.sys.dom/publicApi/differences/<branch-name>";
...
const setName = (branches) =>
  branches.map(({ branchName }) => ({ name: branchName }));
...
export default (panel: vscode.WebviewPanel, workspaceConfig: object) => {
  const getScreenshotDiffs = async ({ branchName }) => {
    if (workspaceConfig.enableMocks) {
      return config.getData(SCREENSHOT_DIFFS_MOCK_FILE).then((data) => {
        vscode.window.showInformationMessage(
          `Returning screenshot diffs mock data for ${branchName}`
        );
        panel.webview.postMessage({
          messageId: "set-screenshot-diffs",
          data: groupByPath(data),
        });
      });
    }
    ...
    try {
      ...
      panel.webview.postMessage({
        messageId: "set-screenshot-diffs",
        data: groupByPath(response),
      });
    } catch (error) {
      console.error(error);
      vscode.window.showErrorMessage(
        `Error fetching screenshot diffs from integration helper - ${branchURL}`
      );
      panel.webview.postMessage({
        messageId: "set-screenshot-diffs",
        data: { files: [], unapproved: 0 },
      });
    }
  };
  ...
  const getAllBranches = async () => {
    if (workspaceConfig.enableMocks) {
      return config.getData(BRANCH_LIST__MOCK_FILE).then((data) => {
        vscode.window.showInformationMessage(
          `Returning mock data for list of branches`
        );
        panel.webview.postMessage({
          messageId: "set-branch-list",
          data: setName(data),
        });
      });
    }
    ...
    try {
      ...
      panel.webview.postMessage({
        messageId: "set-branch-list",
        data: setName(response),
      });
    } catch (error) {
      console.error(error);
      vscode.window.showErrorMessage(
        `Error fetching screenshot diffs from integration helper - ${url}`
      );
