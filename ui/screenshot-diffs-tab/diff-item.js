...
class ScreenshotDiffItem extends React.Component {
    ...
    render() {
        ...
        return (
            <li
                className={`screenshot-diff-item ${
                    isExpanded ? 'expanded' : ''
                } ${approved ? 'is-approved' : ''}`}
                key={diff.path + diff.fileName}
            >
                <h4>
                    {diff.path} <br /> {diff.fileName}
                </h4>
                <button
                    onClick={() =>
                        openFile({ path: diff.path, name: diff.fileName })
                    }
                >
                    See In code
                </button>
                ...
                >
                    <Chevron
                        className={isExpanded ? 'point-down' : 'point-right'}
                    />
                </div>
                {isExpanded && (
                    <React.Fragment>
